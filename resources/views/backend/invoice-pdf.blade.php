            ...
            <div class="table-1">
                <table>
                    <tbody>
                        <tr>
                          @php
                          $company = DB::table('users')->select('company_name')->where('id',$invoice->company)->first();
                          @endphp
                          @if($company)
                          @php $company_name = $company->company_name;@endphp
                          @else
                          ...
                          @endif
                            ...
                            <td>Date :</td>
                            <td><span>{{$invoice->due_date}}</span></td>
                        </tr>
                        <tr>
                            <td>Address :</td>
                            <td><span>{{$invoice->address}}</span></td>
                            <td>Installation Date :</td>
                            <td><span>{{$invoice->completed_at}}</span></td>
                        </tr>
                        <tr>
                        @php 
                        $users = DB::table('users')->select('name')->where('id',$invoice->attention_to)->first();
                        @endphp
                        @if($users)
                        @php $user = $users->name ; @endphp
                        @else
                        @php $user = '-' ; @endphp
                        @endif
                            <td>Attn :</td>
                                    ...
                                    <tbody><tr style="border: none;">
                                            ...
                                            <td style="padding: 0;border:
                                                ...
                                                <span style="padding: 5px;
                                                    display: inline-block;">{{$invoice->phone}}</span>
                                            </td>
                                        </tr>
                                    </tbody></table>
                            </td>
                ...
                <table>
                    ...
                    @endfor
                        ...
                        <tr class="noborder">
                            <td colspan="2"></td>
                            @php 
                            $gst = $invoice->subtotal*$invoice->gst/100;
                            @endphp
                        ...
                        <tr class="noborder">
                            <td colspan="2"></td>
                            @php 
                            $rebates = $invoice->subtotal*$invoice->rebates/100;
                            $total = $invoice->subtotal+$gst-$rebates;
                            @endphp
